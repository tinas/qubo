...
describe('DateAfterOperator', () => {
  const operator = new DateAfterOperator();
  ...
  it('should handle string dates in different formats', () => {
    const date = new Date('2024-01-25T12:00:00Z');
    expect(operator.evaluate(date, '2024-01-24')).toBe(true);
    expect(operator.evaluate(date, '2024-01-24T12:00:00Z')).toBe(true);
    expect(operator.evaluate(date, '2024-01-24T12:00:00+00:00')).toBe(true);
    expect(operator.evaluate(date, '2024-01-25')).toBe(false);
    expect(operator.evaluate(date, '2024-01-26')).toBe(false);
  });

  it('should handle time components with millisecond precision', () => {
    const date = new Date('2024-01-25T12:00:00.001Z');
    expect(operator.evaluate(date, '2024-01-25T12:00:00.000Z')).toBe(true);
    expect(operator.evaluate(date, '2024-01-25T12:00:00.001Z')).toBe(false);
    expect(operator.evaluate(date, '2024-01-25T12:00:00.002Z')).toBe(false);
  });

  it('should handle different timezone offsets', () => {
    const date = new Date('2024-01-25T12:00:00Z'); // UTC noon
    expect(operator.evaluate(date, '2024-01-25T11:59:59+00:00')).toBe(true); // Just before UTC noon
    expect(operator.evaluate(date, '2024-01-25T13:00:00+01:00')).toBe(false); // After UTC noon
  ...
  describe('caching', () => {
    it('should generate different cache keys for different dates', () => {
      ...
      const target = new Date('2024-01-23T12:00:00Z');
    ...
    it('should generate same cache key for same dates', () => {
      const date = new Date('2024-01-25T12:00:00Z');
      const target1 = new Date('2024-01-24T12:00:00Z');
      const target2 = '2024-01-24T12:00:00Z';
